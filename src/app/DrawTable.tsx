...
import { drawStrokes, hasClosedProfile, simplifyStroke, INKS, type Stroke } from '../draw/strokes'
...
import { constructionParts, itemRoute, MATERIAL_LABEL, MATERIALS_FOR_PART, VIEW_LABEL, VIEW_PROMPT, type PartKit } from '../draw/construction'
...
function BuildPreview({ cls, form, parts, kits }: { cls:CraftKey; form:'chair'|'table'|'planter'; parts:ConstructionViews; kits:Record<string,ConstructionPartState> }) {
  ...
  return <section className="build-preview"><div><p className="eyebrow">Your build</p><h3>Turn it around</h3></div>{webgl ? <Canvas camera={{position:[2.8,2.15,3.7],fov:42}} dpr={[1,1]} fallback={unavailable}><ambientLight intensity={1.6}/><directionalLight position={[3,5,4]} intensity={1.8}/><ConvertedItem placed={placed} y={0} physics={false}/><OrbitControls enablePan={false} minDistance={3} maxDistance={6}/></Canvas> : unavailable}<p>These are the same shaped parts and drawn faces that will appear on the island.</p></section>
}
  ...
  return <div className={`item-studio ${route}`}>
    ...
    {route === 'constructed' ? <>
      ...
      <div className="build-layout">
        <section className="drawing-bay focused-bay"><div className="bay-head"><div><span className="step-label">Making your {current?.label.toLowerCase()}</span><h3>{current?.prompt}</h3></div><span>{current?.optional ? 'extra' : 'needed'}</span></div><div className="construction-views" aria-label="Part surfaces">{current?.views.map(v => <button key={v} onClick={() => setView(v)} className={view === v ? 'selected' : ''}>{VIEW_LABEL[v]} <small>{current && partHasProfile(current, v) ? '✓' : ''}</small></button>)}</div><p className="view-brief">{VIEW_PROMPT[view]} Draw one closed shape: it becomes the real {VIEW_LABEL[view].toLowerCase()} silhouette of this {current?.label.toLowerCase()}.</p><div className="part-kit"><span className="kit-label">Shape</span>{(['square','round','tapered','picket','soft'] as const).map(shape=><button key={shape} className={currentKit?.shape===shape?'selected':''} onClick={()=>setKit({shape})}>{shape}</button>)}<span className="kit-label">Material</span>{current && MATERIALS_FOR_PART(current.key).map(material=><button key={material} className={currentKit?.material===material?'selected':''} onClick={()=>setKit({material})}>{MATERIAL_LABEL[material]}</button>)}<label>Width <input aria-label="Part width" type="range" min=".6" max="1.5" step=".05" value={currentKit?.width??1} onChange={e=>setKit({width:Number(e.target.value)})}/></label><label>Depth <input aria-label="Part depth" type="range" min=".5" max="1.5" step=".05" value={currentKit?.depth??1} onChange={e=>setKit({depth:Number(e.target.value)})}/></label><label>Height <input aria-label="Part height" type="range" min=".6" max="1.5" step=".05" value={currentKit?.height??1} onChange={e=>setKit({height:Number(e.target.value)})}/></label>{['#b87945','#d95d39','#4f8fb8','#5c9645','#71747b','#e0a428'].map(color=><button key={color} aria-label={`Part paint ${color}`} className={currentKit?.color===color?'selected kit-color':''} style={{background:color}} onClick={()=>setKit({color,material:'painted-wood'})}/>)}</div><Pad canvasRef={paper} className="paper item-paper" onDown={down} onMove={move} onUp={up} /><ToolRow brush={brush} setBrush={setBrush} color={color} setColor={setColor} eraser={eraser} setEraser={setEraser} gold={gold} active={active} redo={redo} setActive={setActive} setRedo={setRedo} /></section>
        <aside className="build-card"><BuildPreview cls={cls} form={form} parts={parts} kits={kits}/><p className="eyebrow">Your pieces</p><h3>{complete.length} of {required.length} ready</h3><p className="build-help">Pick a piece, shape it, then add your marks.</p><nav className="part-list" aria-label="Construction parts">{partsSpec.map((p, i) => { const done = p.views.every(v => partHasProfile(p, v)); return <button key={p.key} onClick={() => { setSelected(p.key); setView(p.views[0]) }} className={selected === p.key ? 'selected' : ''}><span className={done ? 'part-state done' : 'part-state'}>{done ? '✓' : i + 1}</span><span><b>{p.label}</b><small>{p.optional ? 'Optional' : done ? 'Views drawn' : `${p.views.map(v=>VIEW_LABEL[v]).join(' + ')}`}</small></span></button> })}</nav>
