...
import { assessProfileFit, constructionParts, FORMS_FOR_PART, itemRoute, MATERIAL_LABEL, MATERIALS_FOR_PART, profilePrompt, supportsFor, VIEW_LABEL, type PartKit } from '../draw/construction'
...
function BuildPreview({ cls, form, parts, kits, support, selected, onSelect }: { cls:CraftKey; form:'chair'|'table'|'planter'; parts:ConstructionViews; kits:Record<string,ConstructionPartState>; support:ConstructionSupport; selected:string; onSelect:(part:string)=>void }) {
  ...
  return <section className="build-preview"><div><p className="eyebrow">Your build</p><h3>Pick a part, then turn it around</h3></div><div className="build-stage">{webgl ? <Canvas camera={{position:[2.8,2.15,3.7],fov:42}} dpr={[1,1]} fallback={unavailable}><ambientLight intensity={1.6}/><directionalLight position={[3,5,4]} intensity={1.8}/><ConvertedItem placed={placed} y={0} physics={false} selectedPart={selected} onSelectPart={onSelect}/><OrbitControls enablePan={false} enableZoom={false} rotateSpeed={.55}/></Canvas> : unavailable}</div><p>Click a gold-outlined piece to edit that real part. This is the same assembled object that will appear on the island.</p></section>
}
...
function ItemStudio({ cls, onBack }: { cls: CraftKey; onBack: () => void }) {
  ...
  const fit = current ? assessProfileFit(current, active) : null
  ...
  // A closed primary silhouette is the authored minimum. Secondary views improve
  // the hull but never turn a friendly free-draw task into a tracing checklist.
  const primaryView = (part: typeof partsSpec[number]) => part.views[0]
  const partComplete = (part: typeof partsSpec[number]) => partHasProfile(part, primaryView(part))
  const complete = required.filter(partComplete)
  ...
  const next = partsSpec.slice(currentIndex + 1).find(p => !partComplete(p))
  ...
  const advance = () => { if (!hasClosedProfile(active)) return; if (next) { setSelected(next.key); setView(next.views[0]) } }
