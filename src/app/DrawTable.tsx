  ...
  return <div className={`item-studio ${route}`}>
    ...
    {route === 'constructed' ? <>
      ...
      <div className="build-layout">
        <section className="drawing-bay focused-bay"><div className="bay-head"><div><span className="step-label">Making your {current?.label.toLowerCase()}</span><h3>{current?.prompt}</h3></div><span>{current?.optional ? 'extra' : 'needed'}</span></div><div className="construction-views" aria-label="Part drawing views">{current?.views.map(v => <button key={v} onClick={() => setView(v)} className={view === v ? 'selected' : ''}>{VIEW_LABEL[v]} <small>{(parts[selected]?.[v] ?? []).some(s => !s.erase) ? '✓' : ''}</small></button>)}</div><p className="view-brief">{VIEW_PROMPT[view]} Your drawing stays on this side of the {current?.label.toLowerCase()}.</p><div className="part-kit">{(['square','round','tapered','picket','soft'] as const).map(shape=><button key={shape} className={currentKit?.shape===shape?'selected':''} onClick={()=>setKit({shape})}>{shape}</button>)}<label>Width <input type="range" min=".6" max="1.5" step=".05" value={currentKit?.width??1} onChange={e=>setKit({width:Number(e.target.value)})}/></label><label>Depth <input type="range" min=".5" max="1.5" step=".05" value={currentKit?.depth??1} onChange={e=>setKit({depth:Number(e.target.value)})}/></label>{['#b87945','#d95d39','#4f8fb8','#5c9645','#71747b','#e0a428'].map(color=><button key={color} aria-label={`Part color ${color}`} className={currentKit?.color===color?'selected kit-color':''} style={{background:color}} onClick={()=>setKit({color})}/>)}</div><Pad canvasRef={paper} className="paper item-paper" onDown={down} onMove={move} onUp={up} /><ToolRow brush={brush} setBrush={setBrush} color={color} setColor={setColor} eraser={eraser} setEraser={setEraser} gold={gold} active={active} redo={redo} setActive={setActive} setRedo={setRedo} /></section>
