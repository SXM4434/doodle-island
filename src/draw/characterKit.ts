...
function localMark(ctx:Ctx, strokes:Stroke[], rect:PartRect, signature=false, scale=1, offsetX=0, offsetY=0) { if(!strokes.length)return; const c=document.createElement('canvas');c.width=c.height=256;const g=c.getContext('2d')!
  // Ink lands exactly where the part board showed it: same square art space, same
  // rect stamp, no white sticker halo and no clipping mask eating the player's edges.
  drawStrokes(g,strokes,256);const w=rect.w*(signature?scale:1),h=rect.h*(signature?scale:1),x=rect.x+(rect.w-w)/2+(signature?offsetX:0),y=rect.y+(rect.h-h)/2+(signature?offsetY:0);ctx.drawImage(c,x,y,w,h) }
// One canonical drawing per part follows the kid through every pose (plan §4): the
// renderer re-anchors the same art to each facing's rect. Facing-specific strokes
// from older saves still take precedence when they exist.
function localMarks(ctx:Ctx,c:CharacterConfig,facing:Facing) {
  ...
  for(const part of parts){
    ...
    const marks=m[part]?.[facing] ?? m[part]?.front ?? []
