...
export function convertDrawing(item: Pick<DrawnItem, 'cls' | 'tool' | 'form'>): Conversion {
  ...
  if (archetype) return { language: 'physical', archetype, verb: archetype === 'campfire' ? 'warms your island' : archetype === 'fence' ? 'makes a solid boundary' : archetype === 'chair' ? 'becomes a solid chair' : archetype === 'planter' ? 'becomes a solid planter' : 'becomes a solid table', explanation: 'Each closed part shape you draw is carved into a real 3D piece; the rig assembles those pieces and keeps them stable.' }
