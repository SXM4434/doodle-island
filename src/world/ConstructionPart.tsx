...
function ArtFace({ strokes, view, size }: { strokes: Stroke[]; view: ConstructionView; size: [number,number] }) {
  ...
  const [w,h]=size
  if(view==='front') return <mesh position={[0,0,.001]}><planeGeometry args={[w,h]} /><meshBasicMaterial map={texture} transparent depthWrite={false} /></mesh>
  if(view==='side') return <mesh position={[.001,0,0]} rotation={[0,Math.PI/2,0]}><planeGeometry args={[h,w]} /><meshBasicMaterial map={texture} transparent depthWrite={false} /></mesh>
  return <mesh position={[0,.001,0]} rotation={[-Math.PI/2,0,0]}><planeGeometry args={[w,h]} /><meshBasicMaterial map={texture} transparent depthWrite={false} /></mesh>
}
...
export function ConstructionPart({ kit, views, size, position, rotation=[0,0,0] }: { kit: ConstructionPartState; views: Partial<Record<ConstructionView,Stroke[]>>; size:[number,number,number]; position:[number,number,number]; rotation?:[number,number,number] }) {
  ...
  const faces=<>{views.front&&<ArtFace strokes={views.front} view="front" size={[w,h]} />}{views.side&&<ArtFace strokes={views.side} view="side" size={[d,h]} />}{views.top&&<ArtFace strokes={views.top} view="top" size={[w,d]} />}</>
