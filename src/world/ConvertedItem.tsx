...
export function ConvertedItem({ placed, y }: { placed: Placed; y: number }) {
  ...
  const root = (children: ReactNode) => <group position={[placed.x, y, placed.z]} rotation={[0, placed.rot, 0]}>{children}<primitive object={shadow} position={[0, .03, 0]} /></group>
  ...
  return root(<><mesh position={[0, .72, 0]} material={mats.wood}><cylinderGeometry args={[.94, 1.02, .16, 8]} /></mesh><mesh position={[0, .33, 0]} material={mats.dark}><cylinderGeometry args={[.16, .23, .7, 7]} /></mesh><mesh position={[0, 1.08, .06]} material={emblem}><planeGeometry args={[eW, eH]} /></mesh></>)
}
